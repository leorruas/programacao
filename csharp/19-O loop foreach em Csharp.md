...
O `foreach` é a estrutura de repetição mais limpa e segura do C# quando o objetivo é **ler todos os elementos de uma coleção** (como [[07-Arrays em Csharp|Arrays]], [[09-Coleções em Csharp|Lists]], `Dictionary`, etc.). 

Diferente do `for` tradicional, você não precisa gerenciar um índice (`i`), se preocupar com o tamanho da lista, ou correr o risco de estourar o limite do array (`IndexOutOfRangeException`).
...
> [!WARNING]
> **Você não pode alterar os itens de uma coleção usando a variável de iteração do `foreach`.**
...
Para que qualquer coleção possa usar o `foreach`, ela precisa implementar a interface **`IEnumerable`** (que significa "capaz de ser enumerado"). 
...
| **Performance** | Levemente mais rápido em Arrays básicos | Muito rápido (mínima diferença) | Igual ao `for` |
