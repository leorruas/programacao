...
Imagine um guarda na portaria de um prédio do governo. A [[javascript/01-fundamentos/Funções\|Funções]] dele é binária: ele só pode deixar você entrar (`true`) ou barrar sua entrada (`false`).
...
*   **Valores equivalentes (Truthy e Falsy):** Mas o que acontece se você apresentar outros [[javascript/01-fundamentos/Objetos\|Objetos]] que não são o crachá oficial? O guarda precisa analisar a "validade" do [[javascript/01-fundamentos/Objetos\|Objetos]] para tomar a decisão:
...
5.  [[javascript/01-fundamentos/Tipos de dados\|NaN]] (Not a Number) - Um cálculo matemático inválido.
...
*   `{}` ([[javascript/01-fundamentos/Objetos\|Objetos]] vazio) - Roda como verdadeiro!
...
O operador `!` (NÃO lógico, detalhado em [[javascript/01-fundamentos/Operadores e Operações]]) serve para inverter a avaliação de um valor. Quando você coloca um único `!` antes de qualquer dado, o [[javascript/Introdução ao JavaScript\|JavaScript]] primeiro descobre se ele é truthy ou falsy, e depois devolve o oposto:
...
2.  **Retornos limpos de [[javascript/01-fundamentos/Funções\|Funções]]:** Ao construir uma [[javascript/01-fundamentos/Funções\|Funções]] que precisa responder apenas se algo é válido ou não (como `usuarioLogado()`), usar `return !!token` garante que a [[javascript/01-fundamentos/Funções\|Funções]] devolva estritamente `true` ou `false` em vez de expor a string confidencial do token ou `undefined`.
...
*   **O `!!` altera o dado na memória:** É o ato de arquivar uma **ficha padronizada oficial escrito apenas "true" ou "false"** no banco de dados. Banco de dados e [[javascript/01-fundamentos/API\|API]] frequentemente rejeitam salvar textos ou números soltos em colunas booleanas de sim/não; eles exigem estritamente um valor booleano puro. O `!!` garante que o dado seja convertido de verdade na memória antes de ser salvo ou enviado.
...
*   **Truthy:** Qualquer valor que é avaliado como verdadeiro em um contexto booleano (praticamente tudo, incluindo listas e [[javascript/01-fundamentos/Objetos\|Objetos]] vazios).
